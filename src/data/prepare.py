...
def sum_column(raw_path, interim_path, abrev = "A", timespan = '1D', overwrite = False):
    ...
    nc_dataset[abrev].values = nc_dataset[abrev].values * 1000

    concat_dataset = nc_dataset.resample(valid_time=timespan).sum()

    concat_dataset.to_netcdf(interim_path)

def mean_column(raw_path, interim_path, abrev, timespan='1D', overwrite=False):
